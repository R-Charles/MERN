...
function partition(nums = [], left = 0, right = nums.length - 1) {
    ...
    nums.splice(middle, 1);
    let less=[], larger=[];
    for(let i=0; i<nums.length; i++){
        nums[i] <= temp ? less.push(nums[i]) : larger.push(nums[i]);
    }
    return [...less, temp, ...larger];
}


function partition(nums = [], left = 0, right = nums.length-1){
    let middlePoint = nums[Math.floor((left+right)/2)];
    let i=0, iter=0;
    while(iter<=nums.length){
        if(nums[i] <= middlePoint){
            nums.unshift(nums[i]);
            i++;
        } else {
            nums.push(nums[i]);
